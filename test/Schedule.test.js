...
describe("Schedule", () => {
  describe(tzDate.name, () => {
    it("should be 01:00 (UTC) when 03:00 (Kyiv) in Winter", () => {
      assert.deepStrictEqual(tzDate(new Date("2023-02-06 03:00"), "Europe/Kiev"), new Date("2023-02-06 01:00+00:00"));
    });

    it("should be 00:00 (UTC) when 03:00 (Kyiv) in Summer", () => {
  ...
  describe(Schedule.name, () => {
    describe(Schedule.prototype.getNextMeeting.name, function () {
      ...
      itParam("${value.desc}", getNextMeetingParams, (value) => {
        ScheduleModule.__with__({
          "getNow": () => value.now,
          "Schedule": Schedule,
          "Schedule.prototype.getScheduleData": () => value.scheduleData,
        })(() => {
          assert.deepStrictEqual(new Schedule({getRange: () => {}}).getNextMeeting(), value.expected);
        });
      });
    });
  });
